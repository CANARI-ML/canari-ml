...
class CanariMLBaseDataLoader(DataCollection):
    """Loader base class for all data loaders used in this codebase.
    Based on [IceNet v0.4.0_dev](https://github.com/icenet-ai/icenet/blob/6caa234907904bfa76b8724d8c83cd989230494a/icenet/data/loaders/base.py)

    Attributes:
        loader_configuration: This is a string that will be converted to a
        ...
        generate_workers (bool): Whether to generate workers or not. Only used if
            ...
            worker. If true, then it will return a single `tf.Dataset`. This should
            be set to False when using the distributed strategy and True otherwise
    ...
    def _add_channel_files(self, var_name: str, filelist: object):
        ...
        if var_name in self._channel_files:
            logging.warning(
                "{} already has files, but more found, "
    ...
    def _construct_channels(self):
        ...
        for var_name, meta_channel in self._config["channels"].items():
            ...
            self._add_channel_files(var_name, meta_channel["processed_files"][var_name])
    ...
    def _write_dataset_config(self, counts: object, network_dataset: bool = True):
        """
