...
def create_get_args() -> object:
    """Converts input data creation argument strings to objects, and assigns them as
        attributes to the namespace.

    The args added in this function relate to the dataloader creation process.

    Returns:
        An argparse.ArgumentParser object with all arguments added via `add_argument`
            accessible as object attributes.
    """
    implementations = list(CanariMLDataLoaderFactory().loader_map)
